...
/**
 * @typedef {function(function(Value): Value): LeftContainer} LeftChainFn
 *
 * @sig (Value -> Value) -> LeftContainer
 *
 * `LeftChainFn` is a function that takes a callback function and returns
 * a `LeftContainer`. The callback takes a `Value` and returs a `Value`.
 *
 * `Left(value).chain(fn)` refuses to apply `fn` to the value.
 */

/**
 * @typedef {function(function(Value): Value): Value} RightChainFn
 *
 * @sig (Value -> Value) -> Value
 *
 * `RightChainFn` is a function that takes a callback function and returns
 * a `Value`. The callback takes a `Value` and returs a `Value`.
 *
 * `Right(value).chain(fn)` applies `fn` to `value` and unbox it. It
 * does not return the value inside a right container.
 */
 ...
 * a `LeftContainer`. The callback takes a `Value` and returs a `Value`.
 *
 * `Left(value).map(fn)` refuses to apply `fn` to the value.
 */
