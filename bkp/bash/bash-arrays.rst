...
``man bash`` (`man bash on
arrays <https://www.gnu.org/software/bash/manual/bash.html#Arrays>`__):

   Any element of an array may be referenced using ``${name[*]}``. If
   the subscript is ``@`` or ``*``, the word expands to all members of
   the array name. These subscripts differ only when the word appears
   within double quotes. If the word is double-quoted, ``${name[*]}``
   expands to a single word with the value of each array member
   separated by the first character of the IFS variable, and
...
specifier ``%s`` is reused as necessary to consume all given parameters:
...
thus ``“%s”\n`` is used only once, and the entire output is a single
line inside the curly double quotes.
...
the four output lines.
   ...
   **NOTE**: Using ``[*]`` it is possible to actually convert an array
   into a string.
...
From ``help read``:
   ...
   FD if the -u option is supplied. The line is split into fields as
   with word splitting, and the first word is assigned to the first
   ...
   in $IFS are recognized as word delimiters.
...
Since the ``-a`` option was not used for ``read``, ``IFS='|'`` has no
effect, and the entire input line is assigned as a single word to
``line``.
...
``"$line"`` is used as the parameter to ``printf``, it results in the
first element of the array being retrieved (not all of them). It is
similar to array pointers in C, where the name of the arrays is a
...
Because ``"${line[*]}"`` is used (instead of ``"${line[@]}"``), the
elements were expanded to a single word, and each element is separated
by the first character of IFS, which is a space by default. This
effectively joins the array elements into a single string, with spaces
as the separator.
...
``read`` reads *a single line* from the input source (``help read``).
That is why this example does not loop twice and assign ‘x’, ‘y’, and
‘z’ in turn to ``line``. Rather, it loops only once and assigns the
entire input to ``line``.
...
-  `Bash Manual: Special
   Parameters <https://www.gnu.org/software/bash/manual/bash.html#Special-Parameters>`__

-  `Bash Manual: Word
   Splitting <https://www.gnu.org/software/bash/manual/bash.html#Word-Splitting>`__

-  `Unix StackExchange Question About
   IFS <https://unix.stackexchange.com/questions/26784/understanding-ifs>`__
