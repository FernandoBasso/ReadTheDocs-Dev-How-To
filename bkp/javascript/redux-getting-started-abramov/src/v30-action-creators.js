...
const actionTodoAdd = (text, todosLength) => {
  return {
    type: 'TODO_ADD',
    id: todosLength,
    text,
...
const mapDispatchFilterButtonToProps = (dispatch, ownProps) => {
  return {
    onClick: () => {
      dispatch(actionSetVisibilityFilter(ownProps.filter));
    },
  };
};
...
let AddTodo = ({ dispatch, todosLength }) => {
...
const mapStateAddTodosToProp = (state) => {
  return {
...
const mapDispatchTodoListToProps = (dispatch) => {
  return {
    onTodoClick: (todoId) => {
      dispatch(actionTodoToggle(todoId));
    },
  };
};
